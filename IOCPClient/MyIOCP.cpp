...
void MyIOCP::NotifyDisconnectedClient(ClientContext *pContext)
{
	...
	//m_StatusLock.Lock();
	//if(m_bAutoReconnect)		//s_m: comment
	{
	    // Wait A Litelbit
		//srand((unsigned)time(NULL));			
		//Sleep(rand() % 79); // Prime number 79,59 ,61 ,67 71,89, 97, 101 generates a better uniform value. 
			
		//if (Connect(m_sAutoReconnect, m_iAutoReconnect)) {
		//	BuildPackageAndSendNickname(m_iAutoClientID, m_sAutoNickName);	//s_m
...
BOOL MyIOCP::BuildPackageAndSendNickname(int ClientID, CString sText)
{
	BOOL bRet = FALSE;
	m_ContextMapLock.Lock();
	ClientContext* pContext = FindClient(ClientID);
	...
	if (pContext == NULL)
		return FALSE;
	bRet = BuildPackageAndSendNickname(pContext, sText);
	m_ContextMapLock.Unlock();
	return bRet;
}
...
// Build the a text message message and send it.. 
BOOL MyIOCP::BuildPackageAndSendNickname(ClientContext* pContext, CString sText)
{
	CIOCPBuffer* pOverlapBuff = AllocateBuffer(IOWrite);
	if (pOverlapBuff)
	{
		if (pOverlapBuff->CreatePackage(Job_SendNickname2Client, sText))
			return ASend(pContext, pOverlapBuff);
		else
		{
			AppendLog("CreatePackage(0,sText) Failed in BuildPackageAndSendNickname");
			ReleaseBuffer(pOverlapBuff);
			return FALSE;
		}

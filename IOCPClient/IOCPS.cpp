...
BOOL IOCPS::AddClientContext(ClientContext *mp)
{
	...
	{
		CString msg;
		msg.Format("Duplicate Key! Disconnecting incoming client.");
		AppendLog(msg);
		AbortiveClose(mp);
		m_ContextMapLock.Unlock();
		return FALSE;
...
void IOCPS::FreeClientContext()
{
	...
	while (pos != NULL) 
	{
		...
		if(pContext)
		{
			...
			if (pContext->m_File.m_hFile != INVALID_HANDLE_VALUE)//(unsigned int)
			{
...
BOOL IOCPS::PrepareSendFile(ClientContext *pContext, LPCTSTR lpszFilename)
{
	...
	{    // Already in send mode
		...
		if (pContext->m_File.m_hFile != INVALID_HANDLE_VALUE)//(int)
		{
			pContext->m_File.Close();
		}
...
*/
#if defined TRANSFERFILEFUNCTIONALITY
UINT IOCPS::NotifyFileCompletionProc(LPVOID pParam)
{
	NOTIFYCOMPLETIONINFO* pInfo = (NOTIFYCOMPLETIONINFO*)pParam;
	//s_m:
			/*play a wav file*/

	CString wavFileName = (pInfo->fileName);//(CString*)pParam;
	if (wavFileName.GetLength() > 0) {
		...
		//	}
		//}
		while (waveOut.IsPlaying())
		{
			Sleep(2000);
		}
		monWave.Load(wavFileName);
		...
		/*until here : play a wav file*/
	}
	//delete wavFileName;

	MyIOCP* pThis = (MyIOCP*)(pInfo->pThis);
	ClientContext* pContext = (ClientContext*)pInfo->pContext;
	pContext->m_ContextLock.Lock();
	pThis->BuildPackageAndSend(pContext, "Successfully played!");
	pContext->m_ContextLock.Unlock();

	
	::PostMessage(pThis->m_hWnd, WM_FILERECEIVED, 0, (LPARAM)0);
	return 0;
}

void IOCPS::AddToFile(ClientContext *pContext, DWORD dwIoSize, CIOCPBuffer *pOverlapBuff)
{
	BOOL isCompleted = FALSE;	//s_m
	CString *wavFileName = new CString;	//s_m
	if (pContext->m_File.m_hFile != NULL)
	{
		...
		if(iBytesLeft>=dwIoSize)
		{
			...
			{
				*wavFileName = pContext->m_File.GetFilePath();	//s_m
				pContext->m_File.Close();
				pContext->m_bFileReceivedMode = FALSE;				
			}
			ReleaseBuffer(pOverlapBuff);			
		...
		if (isCompleted && wavFileName->GetLength()>0) {
			//HWND m_hWnd = ((MyIOCP*)this)->m_hWnd;
			
			NOTIFYCOMPLETIONINFO* pInfo = new NOTIFYCOMPLETIONINFO;
			pInfo->fileName = *wavFileName;
			pInfo->pContext = pContext;
			pInfo->pThis = this;
			AfxBeginThread(IOCPS::NotifyFileCompletionProc, (void*)pInfo/*wavFileName*/, THREAD_PRIORITY_NORMAL);
		}
	}
}
#endif 

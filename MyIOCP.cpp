...
#endif
		break;
	case Job_SendNickname2Client:
		PackageNickname(pOverlapBuff, pContext);
		break;
	};
}
...
void MyIOCP::PackageNickname(CIOCPBuffer* pOverlapBuff, ClientContext* pContext)
{
	CString txt = "";
	BYTE type;
	if (pOverlapBuff->GetPackageInfo(type, txt))
	{
		...
		if (pBuffer)
			*pBuffer = pContext->m_Socket;
		::PostMessage(m_hWnd, WM_NEW_NICKNAME, 0, (LPARAM)pBuffer);
	}
}

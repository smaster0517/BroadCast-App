...
ON_MESSAGE(WM_NEW_NICKNAME, OnNewNickname)
...
ON_NOTIFY(LVN_ITEMCHANGED, IDC_CLIENTLIST, &CIOCPDlg::OnLvnItemchangedClientlist)
ON_BN_CLICKED(IDC_SENDFILE2, &CIOCPDlg::OnBnClickedSendfile2)
ON_COMMAND(ID_MENU_RESTORE, &CIOCPDlg::OnMenuRestore)
ON_COMMAND(ID_MENU_EXIT, &CIOCPDlg::OnMenuExit)
ON_WM_SYSCOMMAND()
END_MESSAGE_MAP()
...
LRESULT CIOCPDlg::OnTrayNotification(WPARAM wp, LPARAM lp) {
...
{	
	UpdateData(TRUE);
	int index = m_CtrlClientList.GetNextItem(-1, LVNI_SELECTED);	//s_m		
	if (index < 0)
	{
		AfxMessageBox("None of clients are selected! Select at least one!");
		return;
	}
	int flag = 0;
	for(; ;)//if(m_iCurrentClientID>0)	//s_m	
	{
		...
		for(int i=0;i<20;i++)
		{
			...
			int flag = 0;
			for (int i = 0; i < m_iCurrentClientID.GetSize(); i++) {	//s_m 
				if (m_iocp.BuildPackageAndSend(m_iCurrentClientID[i], m_sSendTxt)) //if (!m_iocp.BuildPackageAndSend(m_iCurrentClientID, m_sSendTxt))
					flag++;
			}
			if(flag == 0)
		...
		*/
		ITEMINFO* pItem = NULL;
		pItem =	(ITEMINFO*)m_CtrlClientList.GetItemData(index);
		if (!pItem || !m_iocp.BuildPackageAndSend(pItem->m_ID, m_sSendTxt))//if (!m_iocp.BuildPackageAndSend(m_iCurrentClientID, m_sSendTxt))
		{
			//DisableClientPanel();	//s_m: comment
			//int SItem = m_CtrlClientList.GetNextItem(-1, LVNI_SELECTED);
			//if (SItem != -1)
			//	m_CtrlClientList.SetItemState(SItem, LVNI_ALL, LVIF_TEXT | LVIF_IMAGE | LVIF_STATE);
			//m_CtrlClientList.SetFocus();

			m_CtrlClientList.SetItemState(index, LVNI_ALL, LVIF_TEXT | LVIF_IMAGE | LVIF_STATE);	//s_m
			char msg[0x100] = { 0 };
			sprintf(msg, "%s%d", "Sending message failed at row :", index);
		...
		index = m_CtrlClientList.GetNextItem(index, LVNI_SELECTED);	//s_m		
		if (index < 0)
			break;
...
{	
	...
	{		
		...
		for (;;)
		{
			...
			pItem = (ITEMINFO*)m_CtrlClientList.GetItemData(index);
			if(pItem)
				m_iocp.DisconnectClient(pItem->m_ID);//m_iocp.DisconnectClient(m_iCurrentClientID);
		}
		UpdateList();
		m_iCurrentClientID=0;
		DisableClientPanel();
...
{	
	...
	}else
	{
		m_iCurrentClientID=0;
		DisableClientPanel();
		//int SItem=m_CtrlClientList.GetNextItem(-1,LVNI_SELECTED);	//s_m : comment
		//if(SItem!=-1)
		//	m_CtrlClientList.SetItemState(SItem,LVNI_ALL, LVIF_TEXT | LVIF_IMAGE | LVIF_STATE);
	}
...
//s_m
LRESULT CIOCPDlg::OnNewNickname(WPARAM wParam, LPARAM lParam)
{
	unsigned int* piID = reinterpret_cast<unsigned int*>(lParam);
	...
	if (pContext != NULL)
	{
		...
		for (int i = 0; i < nCount; i++)
		{
			...
			if (pItem != NULL)
			{
				...
				{
					pContext->m_ContextLock.Lock();
					pItem->m_NickName = pContext->m_strNickname;
					bRepaint = TRUE;
					pContext->m_ContextLock.Unlock();
					m_CtrlClientList.Update(i);
					break;
		...
		pBallon->SetText("New Client accepted! \n" + pContext->m_strNickname + " added in the list!");
...
LRESULT CIOCPDlg::OnClientDisconnected(WPARAM wParam, LPARAM lParam)
{
	...
	{   
		...
		if(pItem!=NULL)
		{
			...
			{
				CTBalloon* pBallon = new CTBalloon(130, 85);
				pBallon->SetText("A Client disconneced! \n" + pItem->m_NickName + " removed in the list!");
				pBallon->SetLifeTime(5);
				pBallon->CreateAndShow();
...
void CIOCPDlg::OnBnClickedSendfile()
{
	...
	if( dlg.DoModal()==IDOK )
	{
		...
		for (;;)
		{
			ITEMINFO* pItem = NULL;
			pItem = (ITEMINFO*)m_CtrlClientList.GetItemData(index);
			if (pItem && m_iocp.BuildFilePackageAndSend(pItem->m_ID/*m_iCurrentClientID*/, sPath/*sFilename*/))	//s_m
			{
			...
			{
				if(pItem)
					m_iocp.DisableSendFile(pItem->m_ID);//m_iocp.DisableSendFile(m_iCurrentClientID);
				char msg[0x100] = { 0 };
				sprintf(msg, "%s%d", "Error could not send file at row :", index);
				AfxMessageBox(msg);
			}
			index = m_CtrlClientList.GetNextItem(index, LVNI_SELECTED);
			if (index < 0)
				break;
		}		
	}
...
void CIOCPDlg::OnLvnItemchangedClientlist(NMHDR* pNMHDR, LRESULT* pResult)
{
...
void CIOCPDlg::OnBnClickedSendfile2()
{
	// TODO: Add your control notification handler code here
	m_AudiogDlg.DoModal();
}

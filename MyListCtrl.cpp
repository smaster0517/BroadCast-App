	...
	if(pDispInfo->item.mask & LVIF_TEXT)// If we whant to show text. 
	{
		...
		if(pItem!=NULL)
		{
			...
			case 0: 
				...
				break;
			case 2:	//3rd column
				::lstrcpy(pDispInfo->item.pszText, pItem->m_NickName);//copy the string.
				break;
...
BOOL MyListCtrl::init()
{
	...
	m_iNumberOfColumns=3;
	m_iColumnWidthArray[0]=40;
	m_iColumnWidthArray[1]=60;
	m_iColumnWidthArray[2]=50;
	m_iColumnWidthArray[3]=10;



	InsertColumn(0,"Address",LVCFMT_LEFT,m_iColumnWidthArray[0]);
	InsertColumn(1,"Status",LVCFMT_LEFT,m_iColumnWidthArray[1]);
	InsertColumn(2,"NickName", LVCFMT_LEFT, m_iColumnWidthArray[2]);
